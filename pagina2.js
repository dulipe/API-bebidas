const search = async(param) =>{
    ...
    const response = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/filter.php?i=${param}`).then(res => res.json())
...
const bebidaInfo = async(param) =>{
    document.querySelector('.resultado').innerHTML = ''
    const response = await fetch(`https://www.thecocktaildb.com/api/json/v1/1/lookup.php?i=${param}`).then(res => res.json())
    for (let i = 0; i < response.drinks.length; i++) {
        // criação das strings e foto
        let drink = response.drinks
